...
<template>
    <div id="options" v-if="tab.currentTab === 1">
        ...
        <div v-if="tab.tabList[tab.currentTab][0] === 0">
            ...
            <div v-if="tab.tabList[tab.currentTab][1] === 0">
                <div class="flex-container" style="flex-direction: row; justify-content: center;">
                    ...
                    <button @click="importSaveList()" class="whiteText fontVerdana generatorButton" style="margin: 0.5vw; border: 0.2vw solid #ffffff; height: 5vw; width: 12vw; font-size: 0.75vw;">
                        Import save list.
                    </button>
                    <button @click="resetTheWholeGame(true)" class="whiteText fontVerdana generatorButton" style="margin: 0.5vw; border: 0.2vw solid #ff0000; height: 5vw; width: 12vw; font-size: 0.75vw; color: #ff0000">
                        Delete save list.
                    </button>
                </div>
                                    ...
                                    <div style="margin: 0.3vw; margin-top: 0.15vw;">
                                        <div style="position: relative; width: 100%; height: 25%;">
                                            <div class="flex-container fontVerdana" style="margin: 0.3vw; margin-top: 0.15vw; height: 20%">
                                                <div style="flex-grow: 1; flex-basis: 0; text-align: left; font-size: 1.2vw;" class="whiteText">{{item.name}}</div>
                                                <div style="flex-grow: 1; flex-basis: 0; text-align: center; font-size: 1.2vw;" class="whiteText">{{item.data.displayVersion}}</div>
                                                <div style="flex-grow: 1; flex-basis: 0; text-align: right; font-size: 1.2vw;" class="whiteText">{{displayModes(item.modes)}}</div>
                                            </div>
                                        </div>
                                        ...
                                        <div style="position: relative; width: 100%; height: 25%; display: flex; align-items: flex-end">
                                            <div style="flex-grow: 0.3333; flex-basis: 0; text-align: left; display: flex; flex-direction: column;" class="fontVerdana">
                                                <span style="font-size: 0.9vw" class="whiteText">{{formatTime(item.data.totalRealTime, 0, 3, 4)}}</span>
                                                <span v-if="game.currentSave != index" style="font-size: 0.75vw" class="whiteText">Offline for {{formatTime((Date.now() - item.data.lastUpdated) / 1000, 0, 3, 4)}}</span>
            ...
            <div v-if="tab.tabList[tab.currentTab][1] === 1">
                <div class="flex-container" style="flex-direction: column; justify-content: center; align-items: center">
                    <span style="font-size: 1.1vw" class="fontVerdana whiteText">Your mode selection is currently: {{displayModesNonOptArray(tmp.saveModes)}}</span>
                    <div class="flex-container" style="flex-direction: row; justify-content: center;">
                        <button @click="createNewSave(tmp.saveModes)" class="whiteText fontVerdana generatorButton" style="margin: 0.5vw; border: 0.2vw solid #ffffff; height: 5vw; width: 12vw; font-size: 0.75vw;">
                            Add new save.
                            <span style="color: #ffff00">WARNING: None of the modes are implemented yet!</span>
                        </button>
                        <button @click="resetModes()" class="whiteText fontVerdana generatorButton" style="margin: 0.5vw; border: 0.2vw solid #ffffff; height: 5vw; width: 12vw; font-size: 0.75vw;">
        ...
        <div v-if="tab.tabList[tab.currentTab][0] === 1">
            <div class="flex-container" style="flex-direction: row; justify-content: center;">
                <button @click="setTimeSpeed()" class="whiteText fontVerdana generatorButton" style="margin: 0.5vw; border: 0.2vw solid #ff0000; height: 5vw; width: 12vw; font-size: 0.75vw; color: #ff0000">
