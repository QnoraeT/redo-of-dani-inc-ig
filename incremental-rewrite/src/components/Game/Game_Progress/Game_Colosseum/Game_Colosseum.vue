...
<template>
    <div id="colosseum" v-if="tab.currentTab === 5">
        <div class="flex-container" style="flex-direction: row; justify-content: center; font-size: 1.4vw; margin-bottom: 1vw;" v-if="Decimal.gte(timesCompleted('nk'), 1)">
            <button @click="switchSubTab(0, 0)" style="color: #f40" class="normalTabButton smallColBorder colButton fontVerdana">Challenges</button>
            <button @click="switchSubTab(1, 0)" style="color: #f40" class="normalTabButton smallColBorder colButton fontVerdana">Research</button>
        </div>
        <div class="flex-container fontVerdana" style="flex-direction: column; justify-content: center; margin-bottom: 1vw;">
            <span style="color: #f40; text-align: center; font-size: 1.2vw;">You have <span style="font-size: 1.4vw;"><b>{{format(player.gameProgress.col.power, 2)}}</b></span> Colosseum Power. <span style="font-size: 1.0vw;">({{format(tmp.col.powGen, 2)}}/s)</span></span>
            <span style="text-align: center; font-size: 1.2vw;" v-bind:style="{ color: player.gameProgress.col.completedAll ? '#0080FF' : '#FF4000' }">This will give you about <span style="font-size: 1.4vw;"><b>{{formatTime(player.gameProgress.col.maxTime, 3)}}</b></span> within challenges.</span>
        </div>
            ...
            <div v-for="item, in COL_CHALLENGES" class="flex-container" :key="item.id">
                <button v-if="item.show" v-bind:class="{ 
                    ...
                    <div v-bind:class="{ colButtonDesc: !completedChallenge(item.id) && !inChallenge(item.id), colButtonDescProg: completedChallenge(item.id) ? false : inChallenge(item.id), colButtonDescComp: completedChallenge(item.id) }" 
                        class="second-cont" 
                        style="height: 33.333%; font-size: 0.7vw"
                    >
                    ...
                    <div v-bind:class="{ colButtonRew: !completedChallenge(item.id) && !inChallenge(item.id), colButtonRewProg: completedChallenge(item.id) ? false : inChallenge(item.id), colButtonRewComp: completedChallenge(item.id) }" 
                        class="third-cont" 
                        style="height: 50%; font-size: 0.7vw"
                    >
            ...
            <div style="display: flex; justify-content: center; flex-direction: row; border: 0.18vw solid #ff4000; height: 39.8vw; width: 50%">
                <div style="width: 100%; height: 100%;">
                    ...
                    <div class="whiteText fontVerdana" style="display: flex; justify-content: center; background-color: #461100; position: relative; width: 100%; height: 58%; font-size: 0.9vw">
                        <span class="centered-text" style="top: 0.4vw">{{COL_RESEARCH[tab.tabList[tab.currentTab][1]].effectDesc(getColResLevel(tab.tabList[tab.currentTab][1]).floor())}}</span>
                        <span class="centered-text" style="top: 1.6vw">{{COL_RESEARCH[tab.tabList[tab.currentTab][1]].effectDescLevel(getColResLevel(tab.tabList[tab.currentTab][1]).floor())}}</span>
                    </div>
