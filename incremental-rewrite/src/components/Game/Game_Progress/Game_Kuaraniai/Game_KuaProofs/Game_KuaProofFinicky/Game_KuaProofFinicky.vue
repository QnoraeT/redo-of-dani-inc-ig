...
<template>
    <div class="flex-vertical font0" style="background-color: #020; border: 0.24vw solid #0f0; padding: 0.6vw; height: 38vw;">
        ...
        <span style="color: #0f0; text-align: center; font-size: 0.75vw">
            You have FKP reset
            <span style="font-size: 0.85vw"><b>{{ format(player.prog.kua.proofs.finicky.times) }}</b></span> 
            times.
        </span>
        ...
        <div class="flex-horizontal" style="margin-top: 0.4vw; flex-wrap: wrap; width: 40vw;">
            <div v-for="(item, index) in KUA_PROOF_UPGS.fkp" :key="index">
                ...
                <button @click="buyKProofUpg(index, 'fkp')" :class="{ nope: !tmp.kua.proofs.upgrades.fkp[index].canBuy, ok: tmp.kua.proofs.upgrades.fkp[index].canBuy}" :style="{ cursor: tmp.kua.proofs.upgrades.fkp[index].canBuy ? 'pointer' : 'not-allowed' }" v-if="item.show.value" style="width: 12vw; height: 8vw; margin-left: 0.15vw; margin-right: 0.15vw; margin-bottom: 0.3vw; font-size: 0.55vw; transition: 0.2s; background-color: #003000" class="font0 whiteText">
                    <span style="margin-right: 0.5vw; color: #0f0; font-size: 0.65vw"><b>{{item.title}}</b></span><span class="whiteText">×{{ format(player.prog.kua.proofs.upgrades.fkp[index]) }}</span><span v-if="Decimal.gt(tmp.kua.proofs.upgrades.fkp[index].freeExtra, 0)">+{{ format(tmp.kua.proofs.upgrades.fkp[index].freeExtra, 2) }}</span><br>
    ...
    <div v-if="tmp.kua.proofs.upgrades.fkp[0].trueLevel.gt(0)" class="flex-container font0" style="background-color: #020; flex-direction: column; border: 0.24vw solid #0f0; padding: 0.6vw; height: 40vw; width: 40%;">
