...
<template>
    ...
    <div v-if="tmp.kua.active.effects">
        ...
        <li v-if="player.gameProgress.kua.upgrades[1] >= 6">Reduces Upgrade 1's super scaling strength by {{ formatPerc(tmp.kua.effects.upg1SuperScaling, 3) }}</li>
        ...
        <li v-if="player.gameProgress.kua.upgrades[1] >= 3">Raises Points gain to ^{{ format(tmp.kua.effects.ptPower, 4) }}</li>
        <li v-if="player.gameProgress.kua.upgrades[1] >= 6">Delays Upgrade 2's softcap by {{ format(tmp.kua.effects.upg2Softcap, 2) }}×</li>
        <li v-if="player.gameProgress.kua.upgrades[0] >= 7">Multiplies Point gain by {{ format(tmp.kua.effects.pts, 2) }}×</li>
        <li v-if="player.gameProgress.kua.upgrades[0] >= 10">Increases KShards' PRai effect boost by ^{{format(tmp.kua.effects.kshardPrai, 4)}}</li>
        <li v-if="player.gameProgress.kua.upgrades[0] >= 10">Boosts KPower gain by ×{{ format(tmp.kua.effects.kpower, 2) }}</li>
        <li v-if="player.gameProgress.kua.upgrades[2] >= 2">Boosts KBlessing gain by ×{{ format(tmp.kua.effects.bless, 2) }}</li>
    </div>
</button>
<button
    ...
    id="autoKua"
    v-if="Decimal.gte(player.gameProgress.pr2.amount, 75) && hasGrowanMilestone(3)"
...
>
    <div class="flex-container" style="flex-direction: column" :style="{ color: gRC(4.5 + Math.sin(gameVars.sessionTime) / 4, 1.0, 1.0), border: `0.18vw solid ${gRC(4.5 + Math.sin(gameVars.sessionTime + Math.PI * 0.5) / 8, 0.5, 1.0)}` }">
        <div style="text-align: center; font-size: 1vw" :style="{ color: gRC(4.5 + Math.sin(gameVars.sessionTime) / 4, 1.0, 1.0) }" class="fontVerdana">
            You have <b>{{ format(player.gameProgress.kua.kshards, 3) }}</b> Kuaraniai shards.
            <div v-if="tmp.kua.active.kshards.effects" class="fontVerdana">
                ...
                <li v-if="Decimal.gt(player.gameProgress.kua.kshards, 0)">Adds Upgrade 5, and makes it's base ×{{format(tmp.kua.effects.upg5, 4)}}/bought.</li>
                <li v-if="player.gameProgress.kua.upgrades[0] >= 2">Multiply PRai's effect by {{ format(KUA_UPGRADES.KShards[1].eff!.value, 2) }}×.</li>
                <li v-if="player.gameProgress.kua.upgrades[0] >= 8">Multiply PRai gain by {{ format(KUA_UPGRADES.KShards[7].eff!.value, 2) }}×.</li>
                <li v-if="player.gameProgress.kua.upgrades[0] >= 9">Delays Upgrade 2's cost growth (after scaling costs) by +{{format(KUA_UPGRADES.KShards[8].eff!.value, 2)}} purchases.</li>
            </div>
        </div>
        <div class="flex-container" style="flex-direction: row; justify-content: center">
            <div v-for="(item, index) in KUA_UPGRADES.KShards" :key="index">
                <Game_KuaUpgrade
                    ...
                    :style="{
                        opacity: 0.5 ** (index - player.gameProgress.kua.upgrades[0]),
                        border: `0.24vw solid ${Decimal.lt(item.cost, player.gameProgress.kua.kshards) ? colorChange('#FF00FF', 1.0, 0.5 + 0.5 * Math.sin(Math.PI * gameVars.sessionTime)) : '#b900ff'}`
                    }"
                    v-if="
                        index >= player.gameProgress.kua.upgrades[0] &&
                        index < player.gameProgress.kua.upgrades[0] + 5 &&
                        item.show
                    "
                    :item="item"
                    :index="index"
    ...
    <div class="flex-container" style="flex-direction: column" :style="{ color: gRC(4.5 + Math.sin(gameVars.sessionTime + Math.PI) / 4, 1.0, 1.0), border: `0.18vw solid ${gRC(4.5 + Math.sin(gameVars.sessionTime + Math.PI) / 8, 0.5, 1.0)}` }">
        <div style="text-align: center; font-size: 1vw" :style="{ color: gRC(4.5 + Math.sin(gameVars.sessionTime + Math.PI) / 4, 1.0, 1.0) }" class="fontVerdana">
            You have <b>{{ format(player.gameProgress.kua.kpower, 3) }}</b> Kuaraniai power.
            <div v-if="tmp.kua.active.kpower.effects" class="fontVerdana">
                ...
                <li v-if="Decimal.gt(player.gameProgress.kua.kpower, 1)">Adds Upgrade 6, and makes it's base +{{format(tmp.kua.effects.upg6, 5)}}/bought.</li>
                <li v-if="player.gameProgress.kua.upgrades[1] >= 1">Increases Upgrade 2's base by +{{format(KUA_UPGRADES.KPower[0].eff!.value, 3)}}.</li>
                <li v-if="player.gameProgress.kua.upgrades[1] >= 2">Makes Upgrade 3 {{ format(KUA_UPGRADES.KPower[1].eff!.value.sub(1).mul(100), 3) }}% more effective.</li>
                <li v-if="player.gameProgress.kua.upgrades[1] >= 4">Delays Upgrade 2's softcap by {{ format(KUA_UPGRADES.KPower[3].eff!.value, 2) }}×.</li>
                <li v-if="player.gameProgress.kua.upgrades[1] >= 5">Raises PRai's effect to ^{{format(KUA_UPGRADES.KPower[4].eff!.value, 4)}}.</li>
            </div>
        </div>
        <div class="flex-container" style="flex-direction: row; justify-content: center">
            <div v-for="(item, index) in KUA_UPGRADES.KPower" :key="index">
                <Game_KuaUpgrade
                    ...
                    :style="{
                        opacity: 0.5 ** (index - player.gameProgress.kua.upgrades[1]),
                        border: `0.24vw solid ${Decimal.lt(item.cost, player.gameProgress.kua.kpower) ? colorChange('#FF00FF', 1.0, 0.5 + 0.5 * Math.sin(Math.PI * gameVars.sessionTime)) : '#b900ff'}`
                    }"
                    v-if="
                        index >= player.gameProgress.kua.upgrades[1] &&
                        index < player.gameProgress.kua.upgrades[1] + 5 &&
                        item.show
                    "
                    :item="item"
                    :index="index"
    ...
    <div v-if="player.gameProgress.unlocks.kb" class="flex-container" style="flex-direction: column" :style="{ color: gRC(4.5 + Math.sin(gameVars.sessionTime + Math.PI) / 4, 1.0, 1.0), border: `0.18vw solid ${gRC(4.5 + Math.sin(gameVars.sessionTime + Math.PI) / 8, 0.5, 1.0)}` }">
        <div style="text-align: center; font-size: 1vw" :style="{ color: gRC(4.5 + Math.sin(gameVars.sessionTime + Math.PI) / 4, 1.0, 1.0) }" class="fontVerdana">
        ...
        <div class="flex-container" style="flex-direction: row; justify-content: center">
            <div v-for="(item, index) in KUA_UPGRADES.Kua" :key="index">
                <Game_KuaUpgrade
                    ...
                    :style="{
                        opacity: 0.5 ** (index - player.gameProgress.kua.upgrades[2]),
                    ...
                    v-if="
                        index >= player.gameProgress.kua.upgrades[2] &&
                        index < player.gameProgress.kua.upgrades[2] + 5 &&
                        item.show
                    "
                    :item="item"
                    :index="index"
