    ...
    <div id="growan" v-if="tab.currentTab === 7">
        <div class="flex-container" style="flex-direction: column">
            <div class="flex-container" style="flex-direction: row; justify-content: center; font-size: 1.4vw; margin-bottom: 1vw;"> 
                ...
                <button @click="switchSubTab(2, 0)" style="color: #fa8" class="normalTabButton smallGroBorder groButton fontVerdana" > 
                    Milestones
                </button>
            </div>
            <span style="text-shadow: #840 0vw 0vw 0.8vw; color: #840; text-align: center; font-size: 1.2vw;" class="fontVerdana">
                You have <span style="font-size: 1.5vw" ><b>{{ format(player.gameProgress.layer4.gro.amount, 3) }}</b></span> grōwan.
            </span>
            <span style="text-shadow: #840 0vw 0vw 0.8vw; color: #840; text-align: center; font-size: 0.8vw;" class="fontVerdana">
                You have <span style="font-size: 1.0vw" ><b>{{ format(player.gameProgress.layer4.gro.totalAmt, 3) }}</b></span> total grōwan, which is multiplying all Grōwan equations by <b><span style="font-size: 1.0vw">{{ format(tmp.layer4.growan.eff.groMult, 2) }}×</span></b> and multiplying Kuaraniai gain by <b><span style="font-size: 1.0vw">{{ format(tmp.layer4.growan.eff.kuaGain, 2) }}×</span></b>.
            </span>
        ...
        <div v-if="tab.tabList[tab.currentTab][0] === 0" class="flex-container" style="flex-direction: column; justify-content: center; margin-bottom: 1vw">
            ...
            <span style="text-shadow: #840 0vw 0vw 0.8vw; color: #840; text-align: center; font-size: 0.8vw; margin-top: 0.2vw" class="fontVerdana">
                You have a best grōwan solution amount of <span style="font-size: 0.9vw" ><b>{{ format(player.gameProgress.layer4.gro.bestGEA) }}</b></span>.
            </span>
            <span style="text-shadow: #840 0vw 0vw 0.8vw; color: #840; text-align: center; font-size: 0.8vw; margin-top: 0.2vw" class="fontVerdana">
                Mult per bought: {{ format(GROWAN_DATA.equ.multPerBought.value, 2) }}×
            </span>
            <span v-if="Decimal.gte(player.gameProgress.layer4.gro.growanEqu[3].bought, 1) || Decimal.gte(player.gameProgress.layer4.gro.equCancel, 1)" style="text-shadow: #840 0vw 0vw 0.8vw; color: #840; text-align: center; font-size: 0.8vw; margin-top: 0.2vw" class="fontVerdana">
        ...
        <div v-if="tab.tabList[tab.currentTab][0] === 1" class="flex-container" style="flex-direction: column; justify-content: center; margin-bottom: 1vw">
            ...
            <div class="flex-container" style="justify-content: center">
                <div class="flex-container" style="flex-direction: column; align-items: center; background-color: #300; border: 0.24vw solid #c00; width: 30vw; margin: 0.25vw">
                    <span class="whiteText fontVerdana" style="font-size: 1.0vw">You have bought {{ player.gameProgress.layer4.gro.upgrades.active.length }} active upgrades.</span>
                    <button
                        ...
                        :style="{
                            cursor: Decimal.gte(player.gameProgress.layer4.gro.amount, item.cost.add(GROWAN_DATA.upgCostModif.active.value)) && !player.gameProgress.layer4.gro.upgrades.active.includes(index)
                                ? 'pointer'
                                : 'not-allowed',
                            backgroundColor: 
                                player.gameProgress.layer4.gro.upgrades.active.includes(index) 
                                    ? '#600' 
                                    : Decimal.gte(player.gameProgress.layer4.gro.amount, item.cost.add(GROWAN_DATA.upgCostModif.active.value)) 
                                        ? '#400' 
                                        : '#200' 
                        }">
                        <span class="fontVerdana whiteText" style="font-size: 0.75vw">{{ item.desc.value }}<br><br></span>
                        <span class="fontVerdana whiteText" style="font-size: 1.0vw">Cost: {{ format(item.cost.add(GROWAN_DATA.upgCostModif.active.value)) }} Grōwan</span>
                    </button>
                </div>
                <div class="flex-container" style="flex-direction: column; align-items: center; background-color: #210; border: 0.24vw solid #840; width: 30vw; margin: 0.25vw">
                    <span class="whiteText fontVerdana" style="font-size: 1.0vw">You have bought {{ player.gameProgress.layer4.gro.upgrades.overall.length }} normal upgrades.</span>
                    <button
                        ...
                        :style="{
                            cursor: Decimal.gte(player.gameProgress.layer4.gro.amount, item.cost.add(GROWAN_DATA.upgCostModif.overall.value)) && !player.gameProgress.layer4.gro.upgrades.overall.includes(index)
                                ? 'pointer'
                                : 'not-allowed',
                            backgroundColor: 
                                player.gameProgress.layer4.gro.upgrades.overall.includes(index) 
                                    ? '#630' 
                                    : Decimal.gte(player.gameProgress.layer4.gro.amount, item.cost.add(GROWAN_DATA.upgCostModif.overall.value)) 
                                        ? '#420' 
                                        : '#210' 
                        }">
                        <span class="fontVerdana whiteText" style="font-size: 0.75vw">{{ item.desc.value }}<br><br></span>
                        <span class="fontVerdana whiteText" style="font-size: 1.0vw">Cost: {{ format(item.cost.add(GROWAN_DATA.upgCostModif.overall.value)) }} Grōwan</span>
                    </button>
                </div>
                <div class="flex-container" style="flex-direction: column; align-items: center; background-color: #023; border: 0.24vw solid #06c; width: 30vw; margin: 0.25vw">
                    <span class="whiteText fontVerdana" style="font-size: 1.0vw">You have bought {{ player.gameProgress.layer4.gro.upgrades.idle.length }} idle upgrades.</span>
                    <button
                        ...
                        :style="{
                            cursor: Decimal.gte(player.gameProgress.layer4.gro.amount, item.cost.add(GROWAN_DATA.upgCostModif.idle.value)) && !player.gameProgress.layer4.gro.upgrades.idle.includes(index)
                                ? 'pointer'
                                : 'not-allowed',
                            backgroundColor: 
                                player.gameProgress.layer4.gro.upgrades.idle.includes(index) 
                                    ? '#036' 
                                    : Decimal.gte(player.gameProgress.layer4.gro.amount, item.cost.add(GROWAN_DATA.upgCostModif.idle.value)) 
                                        ? '#024' 
                                        : '#012' 
                        }">
                        <span class="fontVerdana whiteText" style="font-size: 0.75vw">{{ item.desc.value }}<br><br></span>
                        <span class="fontVerdana whiteText" style="font-size: 1.0vw">Cost: {{ format(item.cost.add(GROWAN_DATA.upgCostModif.idle.value)) }} Grōwan</span>
                    </button>
                </div>
            </div>
        </div>
        <div v-if="tab.tabList[tab.currentTab][0] === 2" class="flex-container" style="flex-wrap: wrap; justify-content: center; margin-bottom: 1vw">
            <div v-for="(item, index) in GROWAN_MILESTONES" :key="index" class="whiteText fontVerdana flex-container" :style="{ backgroundColor: hasGrowanMilestone(index) ? '#632f00' : '#422000' }" style="flex-direction: column; width: 20vw; height: 4.5vw; font-size: 0.65vw; margin: 0.24vw; border: 0.18vw solid #c36100;">
                <span class="fontVerdana whiteText" style="text-align: center">{{ item.desc }}<br><br></span>
                <span class="fontVerdana whiteText" style="text-align: center; font-size: 0.8vw;">Requirement: <b>{{ format(item.req) }}</b> total grōwan</span>
                <!-- <span v-if="tmp.layer4.growan.canDo">Gain <span style="font-size: 1vw"><b>{{ format(tmp.layer4.growan.pending, 3) }}</b></span> grōwan upon grōwanize.<br></span>
                <span v-if="!tmp.layer4.growan.canDo">You need <span style="font-size: 1vw"><b>{{ format(tmp.layer4.growan.nextAt) }}</b></span> Kuaraniai to gain more grōwan.<br></span>
                <span v-if="player.gameProgress.layer4.pickedFirst === 0">Warning: If you grōwan reset, you will be locked out of Taxation!<br></span>
                <span style="font-size: 0.55vw">This is a layer 4 reset, and will reset all prior layers. You can reset even if you won't gain any grōwan.</span> -->
