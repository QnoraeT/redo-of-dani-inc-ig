        ...
        <div v-for="(item, index) in MAIN_UPG_DATA" :key='index'>
            <div class="flex-container" style="flex-direction: column; margin: 0.2vw;" v-if="tmp.main.upgrades[index].active && tmp.main.upgrades[index].shown">
                ...
                class="whiteText mediumButton font0 generatorButton" @click="buyGenUPG(index)">
                    <h3 style="margin-top: 0.35vw; font-size: 0.9vw">Upgrade {{index + 1}}: {{format(player.prog.main.upgrades[index].bought)}}<span style="font-size: 0.55vw" v-if="Decimal.gt(tmp.main.upgrades[index].freeExtra, 0)">&nbsp;(+{{format(tmp.main.upgrades[index].freeExtra)}})</span><span style="font-size: 0.55vw; color: #fa8" v-if="Decimal.gt(player.prog.main.upgrades[index].accumulated, 0) && (inChallenge('dc') || Decimal.gte(timesCompleted('dc'), 11))">&nbsp;(+{{format(player.prog.main.upgrades[index].accumulated)}})</span></h3>
