...
export const TAX_UPGRADES = [
    ...
    },
    {
        ...
        implemented: false,
        cost() { return D(2); },
        get desc() { return `Weaken Upgrade 1's softcap by ${format(5, 1)}% and delay it's hyper scaling by +${format(50, 0)}.`; },
        show: true
    },
    {
        type: 1,
    ...
    },
    {
        ...
        implemented: false,
        cost() { return D(2); },
        get desc() { return `Boost Upgrade 3's base by +${format(0.001, 4)} and decrease it's linear scaling from ${format(100, 1)} to ${format(10, 1)}.`; },
        show: true
    },
    {
        type: 0,
        implemented: false,
        cost() { return D(1); },
        get desc() { return `Unlock the next Colosseum challenge, and delay Upgrade 3's scaling by +${format(15)}.`; },
        show: true
    },
]
    ...
    switch (type) {
        case 0:
            ...
            if (player.value.gameProgress.tax.auto) {
                ...
                player.value.gameProgress.tax.amount = Decimal.add(player.value.gameProgress.tax.amount, generate);
                updateAllTotal(player.value.gameProgress.tax.totals, generate);
                player.value.gameProgress.tax.totalEver = Decimal.add(player.value.gameProgress.tax.totalEver, generate);
            }
