...
<template>
    <div id="taxation" v-if="tab.currentTab === 6">
        <div class="flex-container" style="flex-direction: column; justify-content: center; margin-bottom: 1vw;">
            <span style="text-shadow: #ca0 0vw 0vw 0.8vw; color: #fc0; text-align: center; font-size: 1.2vw;" class="fontVerdana">You have <span style="font-size: 1.5vw;"><b>{{format(player.gameProgress.tax.amount, 3)}}</b></span> taxed coins, which boost your points gain by <b><span style="font-size: 1.5vw;">{{format(tmp.tax.ptsEff, 2)}}x.</span></b></span>
            <button class="whiteText smallTaxBorder taxButton fontVerdana" style="height: 4vw; width: 20vw; font-size: 0.8vw; margin-left: auto; margin-right: auto; margin-top: 1.5vw">
                Gain <span style="font-size: 1vw"><b>{{format(tmp.tax.pending, 3)}}</b></span> taxed coins upon taxation.
                <br>You will do a Colosseum reset.  
            </button>
            <div class="flex-container" style="flex-wrap: wrap; align-content: flex-start; margin-top: 0.8vw; background-color: #430; border: 0.24vw solid #ffd000; margin-left: auto; margin-right: auto; display: flex; justify-content: center; flex-direction: row; padding: 0.6vw; box-shadow: 0 0 0.8vw 0.24vw rgb(66, 55, 0); height: 45vw; width: 80vw">
                <div v-for="(item, index) in TAX_UPGRADES" :key="index" class="flex-container">
                    <!-- set padding to 0vw because it auto-inserts padding -->
                    <button v-bind:style="{ backgroundColor: item.type === 0 ? (Decimal.gte(getTaxUpgrade(index), 1) ? '#847000' : '#634c00') : '#524300' }" v-if="item.show" style="width: 12vw; height: 8vw; margin-left: 0.24vw; margin-right: 0.24vw; font-size: 0.75vw;" class="smallTaxBorder taxButton fontVerdana whiteText">
                        <span v-bind:style="{ color: item.type === 0 ? '#dd0' : '#ff8' }" style="font-size: 0.75vw; margin-right: 0.5vw"><b>#{{index + 1}} [{{item.type === 0 ? 'One-Time' : 'Repeatable'}}]</b></span><br>
                        ...
                        <span v-if="item.type === 0 && Decimal.gte(getTaxUpgrade(index), 1)" class="vertical-align: bottom;"><b><span style="font-size: 0.75vw; color: #ff0">Bought!</span></b></span>
                    </button>
                </div>
            </div>
